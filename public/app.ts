...
type UploadStatus = "idle" | "queued" | "processing" | "review" | "ready" | "failed";
...
const ICONS: Record<string, VNode<ComponentChildren>> = {
...
function App() {
  ...
  const [status, setStatus] = useState("Select a .zip file and start processing.");
  ...
  useEffect(() => {
    const currentUploadId = new URL(window.location.href).searchParams.get("uploadId") || "";
    if (currentUploadId) {
  ...
  const keepOne = useCallback(async (groupId: number, songId: number) => {
    try {
      const response = await fetch(`/api/groups/${groupId}/keep/${songId}`, { method: "POST" });
      const data = (await response.json()) as { keptFiles?: number; error?: string };
      if (!response.ok) {
  ...
  const finalize = useCallback(async () => {
    ...
    try {
      ...
      const data = (await response.json()) as { token?: string; url?: string; expiresAt?: string; error?: string };
      if (!response.ok || !data.url) {
        pushToast("error", data.error || "Finalize failed.");
        return;
      }
      setDownload({ url: data.url, expiresAt: data.expiresAt || new Date(Date.now() + 7 * 86400_000).toISOString() });
      setStatus("Done. Download your de-duplicated archive.");
      ...
      const statusData = (await statusResp.json()) as { upload?: UploadSummary };
          ...
          <form onSubmit=${submit}>
            ...
            <label className="field toggle-field">
              <input
                type="checkbox"
                checked=${autoMode}
                onChange=${(e: Event) => setAutoMode((e.target as HTMLInputElement).checked)}
                disabled=${busy}
              />
            ...
            <section className="panel groups">
              <div className="panel-head">
                <div>
                  ...
                  <p>${summary?.status === "review"
                    ? "Choose which file to keep in each group. Others will be excluded from the download."
                    : "Sorted by similarity. The first file in each group was kept automatically."}</p>
                </div>
              </div>
                    ...
                    <article className="group-card" key=${group.id}>
                      ...
                      <div className="member-grid">
                        ${group.members.map((member) => {
                          ...
                          const reviewState = summary?.status === "review"
                            ? member.keptInOutput
                              ? "selected"
                              : groupHasSelection
                                ? "discarded"
                              ...
                              ${summary?.status === "review"
                                ? html`
                                    <div className="member-state-row">
                                      ${member.keptInOutput
                                        ? html`<span className="kept-label">${ICONS.check} Selected to keep</span>`
                                        : groupHasSelection
                                          ? html`<span className="removed-label">Will be discarded</span>`
                                          : html`<span className="pending-label">Awaiting your choice</span>`}
                                    </div>
                                  `
                                : null}
                              ...
                              ${summary?.status === "review"
                                ? html`
                                    <button
                                      ...
                                      type="button"
                                      onClick=${() => void keepOne(group.id, member.songId)}
                                  ...
                                  `
                                : member.keptInOutput
                                ? html`<span className="kept-label">${ICONS.check} Kept</span>`
                                : html`<span className="removed-label">Excluded</span>`}
                            </div>
                          `;
                        })}
                      </div>
          ...
          (toast) => html`
            <div className=${`toast ${toast.kind}`} key=${toast.id}>
              <div className="toast-mark">
                ${toast.kind === "success" ? ICONS.check : toast.kind === "warning" ? ICONS.info : ICONS.sparkle}
              </div>
              <p>${toast.message}</p>
              <button
